...
A minimal 420coin javascript wallet.

## About

LightWallet is a HD wallet that can store your private keys encrypted in the browser to allow you to run 420coin dapps even if you're not running a local 420coin node. It uses [BIP32][] and [BIP39][] to generate an HD tree of addresses from a randomly generated 12-word seed.
...
Please note that LightWallet has not been through a comprehensive security review at this point. It is still experimental software, intended for small amounts of 420coins to be used for interacting with smart contracts on the 420coin blockchain. Do not rely on it to store larger amounts of 420coins yet.

## Get Started

```
npm install fourtwenty-lightwallet
```

The `fourtwenty-lightwallet` package contains `dist/lightwallet.min.js` that can be included in an HTML page:
...
Concatenates signature object to return signature as hex-string in the same format as `fourtwenty_sign` does.
...
* `txObject.smokeLimit`: Smoke limit
* `txObject.smokePrice`: Smoke price
