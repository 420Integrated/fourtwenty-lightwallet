...
const Util = require('fourtwentyjs-util');
  ...
  describe('signTx', function () {
    it('signs a transaction deterministically', function (done) {
      ...
      KeyStore.createVault({
        ...
        expect(addr).to.equal(fixtures.valid[0].fourtwozerojsTxParams.from);

        const tx = new Transaction(fixtures.valid[0].fourtwozerojsTxParams);
    ...
    it('Correctly handles a 31 byte key from bitcore', function (done) {
      ...
      KeyStore.createVault({
        ...
        keystore.keyFromPassword(password, function (err, pwDerivedKey) {
          ...
          const hexSeedFOURTWENTY = keystore.exportPrivateKey(address, pwDerivedKey);
          const addr0 = KeyStore._computeAddressFromPrivKey(hexSeedFOURTWENTY);
